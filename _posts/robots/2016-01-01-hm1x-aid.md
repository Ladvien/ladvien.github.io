...
The problem is with the `enableUI()` call in the `DataReceivedHandler()` method.  This is where it pays to know how things are being threaded in your program.  It seems the `DataReceivedHandler()` is actually taking place on a separate thread from the main thread, and the main thread is where the UI is being maintained.  This is where the good-ole-Ghostbusters adage is important to recall.

<img height="550" width="265" style="float: left; margin: 10px;" src="https://i.ytimg.com/vi/785gNCpvcQM/maxresdefault.jpg">
