# Creating a Headless Install of Arch Linux for Raspberry Pi Zero W
...
# %wheel        ALL=(ALL)       ALL
```
And uncomment the permissions so it looks like this:
```
## Allows people in group wheel to run all commands
%wheel        ALL=(ALL)       ALL
```

Now add the `alarm` user to the sudoer group

```
usermode -aG sudo alarm
```

## 2. Install needed packages
```
pacman -S gcc git make python2
```

## 3. Install i2c Support
Note, this should _not_ be done as root (no sudo)

Get the user Python wrapper for i2c

[python-smbus-git](https://aur.archlinux.org/packages/python-smbus-git/)

```
git clone https://aur.archlinux.org/python-smbus-git.git
cd python-smbus-git
makepkg -i
```

## 3. Enable the i2c bus as a device

Thank you to douglagm for this tip.
